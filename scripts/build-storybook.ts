...
stories.forEach((info) => {
  ...
  const buildCommand = `yarn dlx cross-env NODE_OPTIONS=--enable-source-maps IS_OVERVIEW=true yarn build-storybook -c ${pkgStorybookConfig} -o ${outDir} --no-manager-cache --preview-url ${path.join(
    basePath,
    'iframe.html'
  )} --force-build-preview`;
